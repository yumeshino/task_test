...
124　koelのダウンロード・作ったファイル（koel）がうまくpushできない
→ダウンロードしたファイルとリポジトリをうまく繋げない
　・koelとういファイルをダウンロードしました（git hubにはサンプルのサイト？とかもダウンロードできるの？）
　・git hubにも同じ名前のリポジトリを作りました
　・その作ったファイルとリポジトリをうまく繋げない
　　→その流れの口頭確認、
　　そもそものファイルとリポジトリの関係
　　一つのファイルを複数のリポジとりで管理とかできるのか？
...
125　udemy_task_test（新しいレポジトリの作成）
・動画のながれでudemy_task_testのリポジトリーを作成して、task_testの階層？でgit initしてるのはなんで？
　→階層的にはそのhtdocksだと思った！

・ローカル側にファイルを追加したいので。。。の意味
　→なんのファイルがどこに追加されるのか
